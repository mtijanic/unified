...
void CResNSS__CResNSSCtor(CResNSS* thisPtr)
{
...
void CResNSS__CResNSSDtor__0(CResNSS* thisPtr)
{
    ...
    uintptr_t address = Platform::ASLR::GetRelocatedAddress(Functions::CResNSS__CResNSSDtor);
    FuncPtrType func = reinterpret_cast<FuncPtrType>    (address);
    int edx = 0;
    func(thisPtr, edx, 2);
}
...
{
    using FuncPtrType = unsigned char*(__fastcall *)(CResNSS*, int);
...
{
    using FuncPtrType = uint32_t(__fastcall *)(CResNSS*, int);
...
{
    using FuncPtrType = int32_t(__fastcall *)(CResNSS*, int);
